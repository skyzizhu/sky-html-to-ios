  ...
  try {
    ...
    const output = {
      ...
      samples: samples.map((sample) => ({
        targetWidthPt: sample.targetWidth,
        sourceProbeWidthCssPx: sample.sourceProbeWidth,
        rootWidthPt: sample.nodes[0].rect.width,
        documentMetrics: sample.documentMetrics,
        nodes: sample.nodes.filter((node) => node.visible).map((node) => ({
          nodeId: node.id,
          parentId: node.parentId,
          selector: node.selector,
          rect: node.rect,
          parentRect: node.parentRect,
