...
def screen_sources(screen: dict[str, Any], ui_stack: str) -> dict[str, str]:
    module_type = str(screen["moduleType"])
    screen_type = str(screen["screenType"])
    if ui_stack == "swiftui":
        return {
            f"{module_type}/Screens/{screen_type}Screen.swift": f'''// Generated by sky-html-to-ios. Native SwiftUI screen for {screen["id"]}.
import SwiftUI

struct HTMLToIOS{screen_type}Screen: View {{
    @ObservedObject var store: HTMLToIOSGeneratedStore
    let screen: HTMLToIOSScreenSpec

    var body: some View {{
        HTMLToIOS{screen_type}ContentView(store: store, screen: screen)
    }}
}}
''',
            f"{module_type}/Views/{screen_type}ContentView.swift": f'''// Generated by sky-html-to-ios. Module-owned SwiftUI content for {screen["id"]}.
import SwiftUI

struct HTMLToIOS{screen_type}ContentView: View {{
    @ObservedObject var store: HTMLToIOSGeneratedStore
    let screen: HTMLToIOSScreenSpec
...
''',
    ...
    return {
        f"{module_type}/Controllers/{screen_type}ViewController.swift": f'''// Generated by sky-html-to-ios. Native UIKit screen for {screen["id"]}.
import UIKit

final class HTMLToIOS{screen_type}ViewController: HTMLToIOSGeneratedScreenViewController {{
    override func wrapGeneratedContent(_ content: UIView) -> UIView {{
        HTMLToIOS{screen_type}ContentView(content: content)
    }}
}}
''',
        f"{module_type}/Views/{screen_type}ContentView.swift": f'''// Generated by sky-html-to-ios. Module-owned UIKit content for {screen["id"]}.
import UIKit

final class HTMLToIOS{screen_type}ContentView: UIView {{
    init(content: UIView) {{
...
def screen_factory_source(screens: list[dict[str, Any]], ui_stack: str) -> str:
    if ui_stack == "swiftui":
        cases = "\n".join(
            f'''        case .{screen["swiftCase"]}:
            HTMLToIOS{screen["screenType"]}Screen(store: store, screen: screen)'''
            for screen in screens
        )
...
'''
    cases = "\n".join(
        f'''        case .{screen["swiftCase"]}:
            return HTMLToIOS{screen["screenType"]}ViewController(screen: screen, actionHandler: actionHandler)'''
        for screen in screens
    )
...
def main() -> int:
    ...
    files = {
        ...
        "Core/Navigation/HTMLToIOSGeneratedScreenFactory.swift": screen_factory_source(screens, ui_stack),
    ...
    for screen in screens:
        files.update(screen_sources(screen, ui_stack))
