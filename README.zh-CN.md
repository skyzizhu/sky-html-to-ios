...
当前生成运行时以 iOS 16 为基线。只有所有必需运行时能力都具备已实现的降级方案时，才允许设置更低 deployment target，否则在代码生成前停止。总控会依据本机 SDK 生成 API 降级计划及设备/Size Class 兼容矩阵，并严格区分来源已分析、iPad 待运行验证、构建通过和视觉验收通过。可选运行 profile 会使用真实 Simulator App 窗口验证手机宽度、横屏、iPad 分屏和常规宽度。
...
关键帧转换会保留采样 offset，以及带方向的 translate、scale、rotate 和 opacity。初始透明但会参与动画的节点仍保留在原生层级中。SVG 内部节点或富文本子节点被合并到一个原生 owner 时，生成后清单会保存可审计的来源映射；仅捕获 SVG 空闲 computed state 时会明确标记为视觉降级，不会冒充完整交互动画。
...
本项目采用 [MIT License](LICENSE) 开源。任何人都可以免费使用、复制、修改、分发、再许可或销售本项目，包括用于商业用途，但必须保留原始版权声明和许可证声明。本项目按现状提供，不附带任何形式的担保。
