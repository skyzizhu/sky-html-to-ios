...
The generated runtime currently has an iOS 16 baseline. A lower deployment target is rejected unless every required runtime capability has an implemented fallback. The pipeline builds an SDK-derived API fallback plan plus a device/Size Class compatibility matrix; source analysis, pending iPad runtime validation, build success, and visual validation remain distinct statuses. Optional runtime profiles validate real Simulator app-window geometry for phone widths, landscape, iPad Split View, and regular width.
...
Backdrop and panel mutations triggered by one action are merged under a single presentation owner instead of becoming duplicate screens or modals. Alert and confirmation content is lowered into native title, message, and default/cancel/destructive actions, and each native button executes the source interaction rather than becoming a display-only placeholder. Closing restores accessibility/input focus to the source control when available. Editable presentation content uses focus-aware system keyboard avoidance, while custom interactive panels use downward velocity/threshold dismissal and yield to inner scrolling until every owned vertical scroll view reaches its top edge. Rapid duplicate presentation requests are suppressed, different nested presentations remain supported, and UIKit action sheets/popovers receive an iPad-safe source anchor.
...
| `native-api-fallback-plan.json` | Required API resolutions and implemented low-version fallbacks |
| `compatibility-matrix.json` | Deployment target, phone/iPad profiles, Size Classes, and source evidence |
