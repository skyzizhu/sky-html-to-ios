...
`sourceRect` 必须先减去所选应用根容器的原点，并显式标记为 `app-root` 坐标系。展示板、嵌套手机画板或 body 带偏移时，禁止直接使用文档绝对坐标。SwiftUI overlay 的 GeometryReader 可能从 Safe Area 内容原点开始，定位前必须用其 global frame 原点把 `app-root` 矩形转换为局部坐标，禁止再次叠加顶部或底部安全距离。运行时应优先保持来源位置，同时用可视区域边界约束整块浮层；不得通过滚动根页面把浮层内部控件“滚到可见”。
