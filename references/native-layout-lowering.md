...
计划按 Screen 保存 Content Container、所有有意义容器、每个节点的盒模型、Flex 证据和复合控件槽位。
...
- gap、alignment、distribution 和 wrap；
...
`content-box` 的固定 min/max 约束在进入原生布局前必须加上 padding 与 border，转换成 border-box 参考值。百分比、`calc()` 和相对单位不得被正则截断成错误常量；其原始契约继续由父容器和响应式规则解析。
...
5. 盒模型尺寸非负且 box sizing 可处理；
6. 复合槽位唯一、完整并保持视觉顺序。

生成后，`native-structure-manifest.json` 再逐容器核对 axis、child order、gap、alignment、distribution，并逐复合控件核对 `compoundLayout` 与 `contentItems`。任一项未消费时不得接入 Xcode target。
