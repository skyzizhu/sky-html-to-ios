...
- `nowrap` 或实测单行的紧凑横向条目必须提高横向 compression resistance；不能为了填满父容器而压窄到换行。
- 只有来源允许换行或多宽度采样证明发生换行时，才让原生文字扩展为多行。不能用统一的 `numberOfLines = 0`/无限行覆盖来源语义。
...
- 为每个容器记录唯一的 axis owner。根纵向页面锁定 vertical，嵌套 carousel 锁定 horizontal；只有来源确实支持二维画布、地图或数据表时才允许 `both`。
- `overflow:auto/scroll` 是允许滚动的证据，`scrollWidth/clientWidth`、`scrollHeight/clientHeight` 和行为 probe 是实际轴向证据。子节点意外超宽不是开启父级横向滚动的理由。
- 横向集合的 item 不参与 viewport 等分，除非 CSS 明确为等分布局。保留 item 的 measured width、`flex-basis`、min/max width、`flex-shrink`、gap 和内容 compression resistance。
- 纵向页面必须约束内容宽度到 viewport/frame layout guide，并关闭无来源的 horizontal bounce/indicator；横向集合则约束内容高度到自身 viewport。
- 长页面至少验证顶部、中段和末端。
- sticky、分页、轮播和下拉刷新分别标记，不能都简化成普通 ScrollView。

## 紧凑视觉元素

- icon、avatar、badge、status slot 和缩略图分别保留 visual container 与内部 glyph/media 尺寸，不能把两者合成一个被父布局拉伸的矩形。
- 来源 rect 接近方形、各 viewport 尺寸稳定且具有背景、边框、圆角或资源时，保留 `width = height` 或来源 aspect ratio。
- `object-fit`/content mode 只控制容器内部资源，不替代容器自身宽高比约束。
